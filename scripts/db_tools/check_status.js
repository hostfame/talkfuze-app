...
async function run() {
  const res = await fetch(`${URL}/messages?content=eq.tui%20ki%20amake%20type%20korte%20dekhtesos%3F&select=id,content,status,platform_message_id`, {
    headers: HEADERS
  });
  const data = await res.json();
  console.log("DB status for outbound msg:", JSON.stringify(data, null, 2));
}
run();
