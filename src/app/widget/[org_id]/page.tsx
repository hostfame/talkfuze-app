            ...
            <div ref={chatScrollContainerRef} onScroll={handleChatScroll} className="flex-1 overflow-y-auto p-5 pb-[120px] flex flex-col gap-3 bg-[#f9fafb]">
              ...
              {(() => {
                ...
                if (isHandoff) {
                  ...
                  return (
                    <div key={idx} className="flex justify-center my-3 select-none">
                ...
                return isAiOrAgent ? (
                  <div key={idx} className="flex flex-col gap-1 items-start mb-1">
                    <div className="flex gap-2 items-end">
                      ...
                      <div className={msg.content_type === 'text' ? `bg-[#f3f4f6] rounded-[18px] ${showAvatar ? 'rounded-bl-[4px]' : ''} py-3 px-4 text-[15px] text-slate-800 max-w-[85%] whitespace-pre-wrap tracking-tight` : "max-w-[85%]"}>
                        {renderMessageContent(msg, false, setLightboxImage)}
                        {msg.metadata?.auto_reply && (
                          <button
                            onClick={handleWhatsAppHandoff}
                            className="mt-3 w-full flex items-center justify-center gap-2 bg-slate-800 text-white py-2.5 px-4 rounded-[12px] text-sm font-semibold hover:bg-slate-900 transition-colors shadow-sm cursor-pointer"
                          >
