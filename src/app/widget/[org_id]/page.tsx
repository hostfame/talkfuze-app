...
export default function WidgetPage() {
  ...
  // isMobileWidget: true when embedded on mobile (< 480px host page width)
  // Passed as ?is_mobile= query param from embed script
  // Controls whether header X close button shows (mobile needs it, desktop uses launcher X)
  const isMobileWidget = searchParams.get('is_mobile') !== 'false'
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      // Double check database state to prevent race conditions with agent replies
      try {
        if (activeConversationId && activeConversationId !== 'new') {
          const latestMsgs = await getWidgetMessages(org_id, deviceId, activeConversationId, Date.now());
          const hasAgentRepliedDB = latestMsgs?.some(m => m.sender_type === 'agent');
          if (hasAgentRepliedDB) {
            console.log("[Auto-reply abort] Agent already replied according to database.");
            return;
          }
        }
      } catch (e) {
        console.error("Failed to check latest messages before auto-reply:", e);
      }
      ...
      const defaultEn = `It seems all our support agents are currently busy. For faster support, please click the button below to message us directly on WhatsApp.`;
      const defaultBn = `সম্ভবত আমাদের সকল সাপোর্ট এজেন্ট এই মুহূর্তে ব্যস্ত আছেন। দ্রুত সাপোর্ট পেতে নিচের বাটনে ক্লিক করে আমাদের হোয়াটসঅ্যাপ এ মেসেজ করুন।`;
          ...
          <div className="relative flex items-center justify-center w-[180px] h-[180px] mb-8">
            ...
            <div className={`w-[120px] h-[120px] rounded-full p-[3px] bg-gradient-to-tr ${isActive ? 'from-emerald-500 to-teal-400 shadow-lg shadow-emerald-500/20' : isRinging ? 'from-amber-500 to-red-500 shadow-lg shadow-amber-500/20' : 'from-[#0070f3] to-blue-400 shadow-lg shadow-blue-500/20'} relative z-20`}>
              <div className="w-full h-full rounded-full bg-slate-900 overflow-hidden flex items-center justify-center">
                <img src="/team/4.avif" className="w-full h-full object-cover bg-white" alt="Support Team" />
              </div>
        ...
        <div className={`absolute inset-0 overflow-hidden bg-white flex flex-col z-40 ${activeTab === 'chat' ? 'flex' : 'hidden'}`}>
            ...
            <div ref={chatScrollContainerRef} onScroll={handleChatScroll} className="flex-1 overflow-y-auto p-5 pb-[130px] flex flex-col gap-1.5 bg-[#f9fafb]">
              ...
              {messages.length === 0 && activeConversationId === 'new' && (
                <div className="flex flex-col gap-2.5 mt-1 ml-[32px] max-w-[85%] animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              {(() => {
                ...
                if (isSystem) {
                  ...
                  if (msg.content.includes('Missed voice call') || msg.content.includes('missed voice call')) {
                    displayContent = 'Missed voice call';
                  } else if (msg.content.includes('voice call') || msg.content.includes('Voice call')) {
                    if (!safeMeta.duration) return null;
                    displayContent = `Voice call ended (${safeMeta.duration})`;
                      ...
                      <div className={msg.content_type === 'text' ? `bg-[#e8eaed] shadow-sm rounded-[18px] ${showAvatar ? 'rounded-bl-[4px]' : ''} py-3 px-4 text-[14.5px] text-slate-800 max-w-[85%] whitespace-pre-wrap tracking-tight` : "max-w-[85%]"}>
                        ...
                        {msg.metadata?.auto_reply && (
                          <button
                            ...
                            className="mt-2.5 w-full flex items-center justify-center gap-2 bg-slate-800 text-white py-2 px-4 rounded-[12px] text-sm font-semibold hover:bg-slate-900 transition-colors shadow-sm cursor-pointer"
                          >
                    ...
                    <div key={msg.id} className="flex flex-col gap-0.5 items-end mb-[1px] mt-[2px]">
                      ...
                      <div className="flex items-center gap-1 mr-0.5">
                        ...
                        {isSending ? (
                          ...
                          <svg className={`w-[15px] h-[15px] ${isSeen ? 'text-blue-500' : 'text-slate-400'}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M2 13l5 5L18 6"/><path d="M8 13l5 5L22 6"/></svg>
                        )}
                      </div>
                    </div>
                  );
