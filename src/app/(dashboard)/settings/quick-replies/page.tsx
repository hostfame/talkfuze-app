...
export default function QuickRepliesSettingsPage() {
  ...
  const handleOpenModal = (reply?: QuickReplyItem) => {
    if (reply) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(`Failed to save quick reply: ${e.message}`)
    } finally {
      setIsSaving(false)
    }
  }
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-4 border-b border-slate-100 dark:border-slate-800">
               <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-full bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center text-[#0070f3] dark:text-blue-400">
                     <Zap size={16} />
                  </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    type="text" 
                    value={shortcutInput}
                    onChange={(e) => setShortcutInput(e.target.value.replace(/[^a-zA-Z0-9_-]/g, '').toLowerCase())}
                ...
                <p className="text-[11px] text-slate-500 mt-1.5">
                  Used to trigger this reply in the chat.
                </p>
              </div>
            ...
            <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex justify-end gap-3">
              <button 
                onClick={() => setIsModalOpen(false)}
                className="px-5 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium transition-colors"
              >
                Cancel
              </button>
              <button 
                ...
                className="flex items-center gap-2 px-6 py-2.5 bg-[#0070f3] hover:bg-blue-600 text-white rounded-lg text-sm font-medium transition-colors disabled:opacity-50 shadow-sm"
              >
